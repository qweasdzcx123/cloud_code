	...
	label:
		...
		for (x = 0; x < S.IN; x++)
		{
			for (y = 0; y < S.ON; y++)
			{
				...
				assign = 0;
				for (m = 0; m < M; m++)
					if (S.IW[m] + (R[m][S.NS[y]] - R[m][S.S[x]]) > B[m])
					{
						assign = 1;
						break;
					}
...
void Initialization()
{
	...
	for (i = 0; i < np; i++)
	{
		...
		{
			for (k = 0; k < M; k++)
				G_BEST.IW[k] = Pbest[i].IW[k];
			for (j = 0; j < N; j++)
				G_BEST.S[j] = Pbest[i].S[j];
			for (j = 0; j < N; j++)
				G_BEST.NS[j] = Pbest[i].NS[j];
			for (j = 0; j < N; j++)
				G_BEST.X[j] = Pbest[i].X[j];
			G_BEST.f = Pbest[i].f;
...
int pop_updating(Solution &off_spring, int theta)
{
	int i, j;
	int f_max = -9999999;
	int f_min = 9999999;
	...
	if ((off_spring).f < f_min)
		return 0; // don't need to update

	for (i = 0; i < np; i++)
	{
		count = 0;
		for (j = 0; j < N; j++)
			if (Pbest[i].X[j] != (off_spring).X[j])
				count++;
		if (count < similarity)
		{
			similarity = count;
	...
	{
		for (j = 0; j < N; j++)
			Pbest[k_closest].X[j] = off_spring.X[j];
	...
	if ((off_spring.f > Pbest[k_closest].f) && (similarity <= theta))
	{
		for (j = 0; j < N; j++)
			Pbest[k_closest].X[j] = off_spring.X[j];
		Pbest[k_closest].f = off_spring.f;
	...
	if ((off_spring.f > f_min) && (similarity > theta))
	{
		for (j = 0; j < N; j++)
			Pbest[k_min].X[j] = off_spring.X[j];
	...
	while (iter < 5000) // 5000 is the maximum number of iterations which is set according to the size of instances
	{
		for (m = 0; m < np; m++)
		{
			...
			while (1)
			{
				...
				count++;
				if (count > 10)
					break; // 10 represents the number of neighbors of particles
			}

			for (j = 0; j < N; j++)
				PY[j] = a * Pbest[m].X[j] + b * (1 - Pbest[m].X[j]);
			for (j = 0; j < N; j++)
				GY[j] = a * Pbest[K].X[j] + b * (1 - Pbest[K].X[j]);
			for (j = 0; j < N; j++)
				QPOP[m][j] = (e1 * QPOP[m][j] + e2 * PY[j] + e3 * GY[j]);
	...
	for (int t = 0; t < 30; t++)
	{
		...
		for (i = 0; i < Nruns; i++) // perform the DQPSO algorithm Nruns times
		{
			...
			if (G_BEST.f > BestResult)
			{
				...
				Nhit = 1;
				for (j = 0; j < N; j++)
					SS_BEST.S[j] = G_BEST.S[j];
				for (j = 0; j < N; j++)
					SS_BEST.NS[j] = G_BEST.NS[j];
				for (j = 0; j < N; j++)
					SS_BEST.X[j] = G_BEST.X[j];
				for (j = 0; j < M; j++)
					SS_BEST.IW[j] = G_BEST.IW[j];
				SS_BEST.f = G_BEST.f;
